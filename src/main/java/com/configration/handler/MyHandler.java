package com.configration.handler;
...
public class MyHandler  extends AbstractUriTemplateHandler{
	...
	protected URI expandInternal(String uriTemplate, Object... uriVariables) {
		...
		UriComponents uriComponents = expandAndEncode(uriComponentsBuilder, uriVariables);
		return createUri(uriComponents);*/
		//UriTemplateBuilder ub=UriTemplate.buildFromTemplate(uriTemplate);
		StringBuffer sb=new StringBuffer();
		sb.append(uriTemplate);
		for (int i = 0; i < uriVariables.length; i++) {
			sb.append("{&"+i+"*}");
		}
		String sbUriStr=sb.toString();
		if (!sbUriStr.contains("?")){
			sbUriStr=sbUriStr.replaceFirst("&", "?");
		}
		URI uri=null;
		try {
			UriTemplate uriRebuilder=UriTemplate.fromTemplate(sbUriStr);
					for (int i = 0; i < uriVariables.length; i++) {
						uriRebuilder.set(""+i, uriVariables[i]);
					}		
			uri = new URI(uriRebuilder.expand());
